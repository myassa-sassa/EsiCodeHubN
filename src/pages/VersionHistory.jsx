  ...
  return (
    <div className="page-container">
      ...
      <div className="stats-cards">
        {[['📊', versions.length,'Total Versions'],['👨‍🎓',4,'Students'],['🔄','2.5','Avg Versions']].map(([icon,val,label]) => (
      ...
      <table className="data-table">
        <thead>
          <tr>
            <th style={{ width:50 }}>🔘</th><th>👨‍🎓 STUDENT</th><th>📝 TITLE</th>
            <th>🔢 VERSION</th><th>📝 CHANGES</th><th>📅 DATE</th><th>⚡ ACTION</th>
          </tr>
        </thead>
        <tbody>
          {versions.map(v => (
            <tr key={v.id}>
              ...
              <td>{v.date}</td>
              <td>
                <button className="review-btn" onClick={() => alert(`Viewing ${v.studentName} — ${v.version}`)}>
                  📖 View
                </button>
              </td>
            </tr>
