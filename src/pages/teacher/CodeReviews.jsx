...
import { statusStyle, modColor } from "../../utils/helpers";
...
export function CodeReviews() {
  ...
  useEffect(() => {
    ...
    if (r) {
      setModal(JSON.parse(r));
      setIsView(false);
      localStorage.removeItem('reviewingCode');
    } else if (v) {
      setModal(JSON.parse(v));
      setIsView(true);
      localStorage.removeItem('viewingCode');
    }
  }, []);
          ...
          {reviews.map(r => {
            ...
            return (
              <tr key={r.id}>
                ...
                <td>{r.title}</td>
                <td>
                  <span className="module-badge" style={modColor(r.module)}>
                    {r.module}
                  </span>
                </td>
                <td>
